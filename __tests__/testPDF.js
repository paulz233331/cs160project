...
describe("Testing with Jest", () => {
  ...
  test("Resume field", () => {
    ...
    let rawdata = fs.readFileSync('compiled/resumePdf.json');
    let applicant = JSON.parse(rawdata);
    var expectedName = "Alex Dubinchyk";
    var expectedEmail = "alexs.dbk@gmail.com";
    var expectedObjective = "Seeking a challenging position to use my software Web development and process optimization\nskills.";
    var expectedSummary = "I worked on a wide range of products including building advanced dynamic multi language web\n"
    ...
    " users roles(RBAC), online editors(logs, statistical formulas, projects rules and etc) for\nmore 10k clients.\n"+
    "Finalization coding script of internal protection algorithm authorization and validation.\nProgramming API service"+
    " for quality control assessment. Interact with user interface(AJAX)\nwith fast load up JSON data, audio files "+
    "and extract large data in excel.\nAPI data exchange integrated with data parse, merge, view in table linkage,"+
